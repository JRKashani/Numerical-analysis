...
% Outputs results to 'comparison_results.txt'.
...
if fileID == -1
    error('Cannot open file comparison_results.txt for writing.');
end

% Helper function to print to BOTH console and file
print_dual = @(fid, fmt, varargin) fprintf(1, fmt, varargin{:}) + fprintf(fid, fmt, varargin{:}); 
% Note: Using anonymous function to avoid code duplication

header = sprintf([...
    '==============================================================\n' ...
    '               PROJECT 1: SOLVER COMPARISON                   \n' ...
    '==============================================================\n']);
print_dual(fileID, '%s', header);
...
print_dual(fileID, '%-6s | %-4s | %-12s | %-12s | %-10s\n', ...
...
for i = 1:size(test_cases, 1)
    ...
    print_dual(fileID, '%6.1f | %4d | %12.6f | %12.6f | %10.2e\n', ...
            Bi, m, x_manual, x_ai, diff_val);
end

print_dual(fileID, '\n\n--- PART 2: PERFORMANCE SPEED TEST (10,000 Runs) ---\n');
Bi_speed = 0.1 + (60 - 0.1) * rand(); 
% m integer between 1 and max_m
m_speed = randi([1, 100]);
num_runs = 10000;

print_dual(fileID, 'Benchmarking for Bi=%.1f, m=%d...\n', Bi_speed, m_speed);
...
print_dual(fileID, 'Manual Code Total Time:  %8.4f seconds\n', time_manual);
print_dual(fileID, 'AI Code Total Time:      %8.4f seconds\n', time_ai);
print_dual(fileID, 'Speedup Factor:          %.2fx faster\n', ...
    time_manual / time_ai);

% Close file
fclose(fileID);
fprintf('\nResults successfully saved to comparison_results.txt\n');
